...
模型与联网配置请在应用的“设置 → 模型与联网”中填写。配置只保存在当前用户目录的 `~/.mlca/api_config.local.py`，不会进入安装包、公开 Release 或源码归档；安装和升级也不会覆盖已经存在的用户配置。
...
| 版本 | 类型 | 重点 |
|------|------|------|
| **0.15.4** | patch | Cowork 最终答复去重、准备阶段与长工具组折叠、任务进度累计；模型/联网配置改为用户目录独立保存并增加发布零携带校验 |
| **0.15.3** | patch | 明亮模式工作区层级调整：浅绿色环绕顶部与左右区域，中间对话区域改为白色 |
